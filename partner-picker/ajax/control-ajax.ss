            ...
            (with-handlers 
                ([exn:fail? 
                  (lambda (exn)                 
                    (redirect/get)
                    (show-new-profile #:picker picker
                                      #:error "One of the fields was filled incorrectly"
                                      ))])
              (show-profile #:picker picker
...
(define (show-profile #:picker picker
                      ...
                      #:view [view "requests"])
  (local [(define (response-generator make-url)
            (render-profile #:availability-url (javascript: (show "availability"))
                            #:error err-msg
                            #:firstname (user-first-name user)
                            #:lastname (user-last-name user)
                            #:section (user-section user)
                            #:email (user-email user)
                            #:requests-url (javascript: (show "requests"))
                            #:make-accept-request-url (lambda (info)
                                                        (make-url (accept-request-handler info)))
                            #:make-submit-request-url (lambda (info)
                                                        (make-url (submit-request-handler info)))
                            ...
                            #:availability (make-availability-table user)
                            #:script script
                            #:make-url (lambda (info)
                                         (make-url (change-time-handler info)))
                            #:view view))
          
          (define script
            (javascript
             (function show (divName)
                       (if (== divName "availability")
                           (block (= (field (field ((field document getElementById) "availability") style) visibility) "visible")
                                  (= (field (field ((field document getElementById)"requests") style) visibility) "hidden"))
                           (block (= (field (field ((field document getElementById)"availability") style) visibility) "hidden")
                                  (= (field (field ((field document getElementById)"requests") style) visibility) "visible"))))))
          
          (define (requests-handler request)
            (with-handlers
                ([exn:fail?
                  (lambda (exn)
                    (show-profile #:error "Internal error unable to show requests at this time"
                                  #:picker picker
                                  #:user user))])
              (show-profile #:picker picker
                            #:user user)))
          ...
          (define (availability-handler request)
            (with-handlers 
                ([exn:fail?
                  (lambda (exn)                                    
                    (show-profile #:error "Internal error unable to show availability at this time"
                                  #:picker picker
                                  #:user user))])
          ...
          (define (request-status-handler info)
            (lambda (request)
              (with-handlers 
                  ([exn:fail?
                    (lambda (exn)                                    
                      (show-profile #:error "Unable to change request status."
                                    #:picker picker
                                    #:user user))])
                (show-profile #:picker picker
                              #:user user
          ...
          (define (change-time-handler info)
            (lambda (request)
              (with-handlers 
                  ([exn:fail?
                    (lambda (exn)                                    
                      (show-availability #:error "Unable to change times specified."
                                         #:picker picker
                                         #:user user))])
          ...
          (define (submit-request-handler info)
            (lambda (request)
              (with-handlers 
                  ([exn:fail?
                    (lambda (exn)                                    
                      (show-requests #:error "Unable to make specified request."
                                     #:picker picker
                                     #:user user))])
          ...
          (define (accept-request-handler info)
            (lambda (request)
              (with-handlers 
                  ([exn:fail?
                    (lambda (exn)                                    
                      (show-requests #:error "Unable to accept specified request."
                                     #:picker picker
                                     #:user user))])
