...
(define (show-profile #:picker picker
                      ...
                      #:user user)
  (local [(define (response-generator make-url)
            (render-profile #:availability-url (make-url availability-handler)
                            #:error err-msg
                            #:firstname (user-first-name user)
                            #:lastname (user-last-name user)
                            #:section (user-section user)
                            #:email (user-email user)
                            #:requests-url (make-url requests-handler)))
...
(define (show-requests #:picker picker
  ...
  (local [(define (response-generator make-url)
            (render-requests
             ...
             #:availability-url (make-url availability-handler)
          ...
          (define (submit-request-handler info)
            (lambda (request)
              (with-handlers 
                ...
                (redirect/get)
                (show-requests #:picker picker;need to add a redirect/get
                               #:user user))))
